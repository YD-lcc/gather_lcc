...
echo "$ACTION devname:$DEVNAME devpath:$DEVPATH devicename:$DEVICENAME devnum:$DEVNUM driver:$DRIVER type:$TYPE product:$PRODUCT seqnum:$SEQNUM busnum:$BUSNUM major:$MAJOR $MINOR  $SUBSYSTEM" > /root/log.txt
...
if echo $DEVPATH | grep -q ff5c0000; then
	...
	echo $wwandevice | grep -q $ifname && echo "$DEVNAME $wwandevice device is already ok" >> /root/log.txt && exit 0
